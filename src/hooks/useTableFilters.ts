...
import { shouldFilterByDIC, shouldFilterByICO } from '../validation/filterValitation';
...
export function useTableFilters<T extends Record<string, any>>(
  ...
  return useMemo(() => {
    return data.filter((item) => {
      ...
      if (filters.vat_rate && Array.isArray(filters.vat_rate) && filters.vat_rate.length > 0) {
        if (!filters.vat_rate.includes(item.vat_rate)) return false;
      }

      // Unit of measure filter (multiselect)
      if (filters.unit_of_measure && Array.isArray(filters.unit_of_measure) && filters.unit_of_measure.length > 0) {
        if (!filters.unit_of_measure.includes(item.unit_of_measure)) return false;
      }

      // Sales group filter
      const salesGroupFilter = filters.sales_group || [];
      if (salesGroupFilter.length > 0) {
        if (!item.sales_group || !salesGroupFilter.includes(String(item.sales_group))) {
          return false;
        }
      }
