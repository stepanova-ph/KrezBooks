...
const TableControlsContext = createContext<TableControlsContextValue<any> | null>(null);
...
export function TableControlsProvider<T>({ children, disabled = false }: TableControlsProviderProps<T>) {
  ...
  const getKeyRef = useRef<((item: T) => string | number)>((item: any) => item.id);
  ...
  const setData = useCallback((data: T[], getKey: (item: T) => string | number) => {
...
export function useTableControls<T>(): TableControlsContextValue<T> {
  ...
  if (!context) {
    throw new Error('useTableControls must be used within TableControlsProvider');
  }
  return context;
}