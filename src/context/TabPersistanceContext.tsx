...
const TabPersistenceContext = createContext<TabPersistenceContextValue | null>(
	null,
);
...
export function TabPersistenceProvider({
	children,
}: TabPersistenceProviderProps) {
	const [filterStates, setFilterStates] = useState<Partial<TabFilterStates>>(
		{},
	);
	const [invoiceFormState, setInvoiceFormState] =
		useState<InvoiceFormState | null>(null);

	const getFilterState = useCallback(
		(tab: keyof TabFilterStates) => {
	...
	const setFilterState = useCallback(
		(tab: keyof TabFilterStates, state: FilterState) => {
...
export function useTabPersistence() {
	...
	if (!context) {
		throw new Error(
			"useTabPersistence must be used within TabPersistenceProvider",
		);
	}
	return context;
}
