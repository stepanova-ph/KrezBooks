...
describe("ContactService", () => {
	...
	describe("update", () => {
		...
		it("should throw error when no fields to update", async () => {
			...
			await expect(contactService.update("20202020", 1, {})).rejects.toThrow(
				"No fields to update",
			);
		});
	});
	...
	describe("delete", () => {
		...
		it("should throw error when deleting non-existent contact", async () => {
			await expect(contactService.delete("99999999", 1)).rejects.toThrow();
		});
		...
		it("should not delete contact with wrong modifier", async () => {
			...
			await expect(contactService.delete("40404040", 2)).rejects.toThrow();
	...
	describe("price_group validation", () => {
		it("should accept price_group values 1-4", async () => {
			for (let priceGroup = 1; priceGroup <= 4; priceGroup++) {
				...
				const retrieved = await contactService.getOne(
					`${priceGroup}0000000`,
					1,
				);
