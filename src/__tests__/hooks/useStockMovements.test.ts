...
import type { StockMovement, CreateStockMovementInput } from '../../../src/types/database';
...
describe('useStockMovement', () => {
  ...
  describe('useStockMovementsByInvoice', () => {
    it('fetches movements for specific invoice', async () => {
      ...
      const { result } = renderHook(
        () => useStockMovementsByInvoice('INV-001'),
        { wrapper: createWrapper() }
      );
      ...
      expect(result.current.data).toEqual(mockMovements);
      expect(mockElectronAPI.stockMovements.getByInvoice).toHaveBeenCalledWith('INV-001');
    });
    ...
    it('does not fetch when invoice number is empty', () => {
      ...
      expect(mockElectronAPI.stockMovements.getByInvoice).not.toHaveBeenCalled();
    });
    ...
    it('returns empty array when invoice has no movements', async () => {
      ...
      const { result } = renderHook(
        () => useStockMovementsByInvoice('INV-002'),
        { wrapper: createWrapper() }
      );
  ...
  describe('useCreateStockMovement', () => {
    it('creates stock movement successfully', async () => {
      ...
      expect(mockElectronAPI.stockMovements.create).toHaveBeenCalledWith(newMovement);
  ...
  describe('useStockAmountByItem', () => {
    it('fetches stock amount for specific item', async () => {
      ...
      expect(mockElectronAPI.stockMovements.getStockAmountByItem).toHaveBeenCalledWith('1234567890123');
    });
