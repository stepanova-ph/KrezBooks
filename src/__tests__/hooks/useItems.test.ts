...
describe("useItems", () => {
	...
	describe("useItem", () => {
		it("fetches single item successfully", async () => {
			...
			const { result } = renderHook(() => useItem("1234567890123"), {
				wrapper: createWrapper(),
			});
			...
			expect(mockElectronAPI.items.getOne).toHaveBeenCalledWith(
				"1234567890123",
			);
		});
	...
	describe("useDeleteItem", () => {
		it("deletes item successfully", async () => {
			...
			expect(mockElectronAPI.items.delete).toHaveBeenCalledWith(
				"1234567890123",
			);
			expect(result.current.data).toEqual({ changes: 1 });
		});
