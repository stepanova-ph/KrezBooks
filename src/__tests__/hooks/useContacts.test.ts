...
describe("useContacts", () => {
	...
	describe("useContact", () => {
		it("fetches single contact successfully", async () => {
			...
			const { result } = renderHook(() => useContact("12345678", 1), {
				wrapper: createWrapper(),
			});
			...
			expect(mockElectronAPI.contacts.getOne).toHaveBeenCalledWith(
				"12345678",
				1,
			);
		});
		...
		it("does not fetch when ico or modifier is missing", () => {
			const { result } = renderHook(() => useContact("", 0), {
				wrapper: createWrapper(),
			});
	...
	describe("useDeleteContact", () => {
		it("deletes contact successfully", async () => {
			...
			expect(mockElectronAPI.contacts.delete).toHaveBeenCalledWith(
				"12345678",
				1,
			);
			expect(result.current.data).toEqual({ changes: 1 });
