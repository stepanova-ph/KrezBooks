...
describe('itemSchema', () => {
  ...
  describe('ean validation', () => {
    it('should accept 13 character EAN', () => {
      const result = itemSchema.safeParse({ ...validItem, ean: '1234567890123' });
      expect(result.success).toBe(true);
    });
  ...
  describe('unit_of_measure validation', () => {
    it('should accept valid units', () => {
      ...
      for (let unit of units) {
        const result = itemSchema.safeParse({ ...validItem, unit_of_measure: unit });
        expect(result.success).toBe(true);
      }
    });

    it('should require at least 1 character', () => {
      const result = itemSchema.safeParse({ ...validItem, unit_of_measure: '' });
      expect(result.success).toBe(false);
    });

    it('should reject units over 20 characters', () => {
      const longUnit = 'A'.repeat(21);
      const result = itemSchema.safeParse({ ...validItem, unit_of_measure: longUnit });
      expect(result.success).toBe(false);
    });
  });
  ...
  describe('price validation', () => {
    ...
    it('should convert string prices to numbers', () => {
      ...
      expect(result.success).toBe(true);
      if (result.success) {
        expect(result.data.sale_price_group1).toBe(100.50);
