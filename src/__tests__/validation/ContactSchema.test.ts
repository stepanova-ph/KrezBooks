...
describe('contactSchema', () => {
  ...
  describe('modifier validation', () => {
    it('should accept modifiers 1-100', () => {
      for (let mod of [1, 50, 100]) {
        const result = contactSchema.safeParse({ ...validContact, modifier: mod });
        expect(result.success).toBe(true);
      }
    });
    ...
    it('should reject modifier 101', () => {
      const result = contactSchema.safeParse({ ...validContact, modifier: 101 });
      expect(result.success).toBe(false);
    });

    it('should convert string modifier to number', () => {
      const result = contactSchema.safeParse({ ...validContact, modifier: '5' });
      expect(result.success).toBe(true);
      if (result.success) {
  ...
  describe('price_group validation', () => {
    it('should accept price groups 1-4', () => {
      for (let pg of [1, 2, 3, 4]) {
        const result = contactSchema.safeParse({ ...validContact, price_group: pg });
        expect(result.success).toBe(true);
      }
    });

    it('should reject price group 0', () => {
      const result = contactSchema.safeParse({ ...validContact, price_group: 0 });
      expect(result.success).toBe(false);
    });

    it('should reject price group 5', () => {
      const result = contactSchema.safeParse({ ...validContact, price_group: 5 });
      expect(result.success).toBe(false);
    });
  });
  ...
  describe('company_name validation', () => {
    it('should require at least 2 characters', () => {
      const result = contactSchema.safeParse({ ...validContact, company_name: 'A' });
      expect(result.success).toBe(false);
    });

    it('should accept 2-40 characters', () => {
      const result = contactSchema.safeParse({ ...validContact, company_name: 'AB' });
      expect(result.success).toBe(true);
    });

    it('should reject over 40 characters', () => {
      const longName = 'A'.repeat(41);
      const result = contactSchema.safeParse({ ...validContact, company_name: longName });
      expect(result.success).toBe(false);
    });
  });
