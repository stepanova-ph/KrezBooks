...
import { contactService, itemService, invoiceService, stockMovementService } from '../../service';
import type { CreateContactInput, CreateItemInput, CreateInvoiceInput, CreateStockMovementInput } from '../../types/database';

// Helper functions to create test data
async function createTestContact(ico: string = '12345678', modifier: number = 1) {
...
describe('Invoice and Stock Movement Integration', () => {
  ...
  describe('Invoice Creation', () => {
    it('should create invoice via IPC wrapper', async () => {
      ...
      const response = await handleIpcRequest(() => invoiceService.create(invoice));
  ...
  describe('Stock Movement Creation', () => {
    ...
    it('should create stock movement via IPC wrapper', async () => {
      ...
      const response = await handleIpcRequest(() => stockMovementService.create(movement));

      expect(response.success).toBe(true);
      expect(response.data?.changes).toBe(1);

      // Verify it's in the database
      const retrieved = await stockMovementService.getOne('INV-100', '1234567890123');
      expect(retrieved).toBeDefined();
  ...
  describe('Complete Invoice with Stock Movements', () => {
    it('should create invoice and multiple stock movements', async () => {
      ...
      const invoiceResponse = await handleIpcRequest(() => invoiceService.create(invoice));
      ...
      const invoiceMovements = await stockMovementService.getByInvoice('INV-200');
    ...
    it('should delete invoice and cascade delete stock movements', async () => {
      ...
      // Delete invoice
      const deleteResponse = await handleIpcRequest(() => invoiceService.delete('INV-300'));
  ...
  describe('Stock Calculations', () => {
    ...
    it('should calculate total stock amount', async () => {
      const amount = await stockMovementService.getStockAmountByItem('1234567890123');
      expect(amount).toBe(30);
    });

    it('should calculate average buy price', async () => {
      const avgPrice = await stockMovementService.getAverageBuyPriceByItem('1234567890123');
      // (10*50 + 20*60) / 30 = 1700/30 = 56.67
      expect(avgPrice).toBeCloseTo(56.67, 2);
    });

    it('should get last buy price', async () => {
      const lastPrice = await stockMovementService.getLastBuyPriceByItem('1234567890123');
      expect(lastPrice).toBe(60.00);
    });

    it('should return 0 for item with no movements', async () => {
      await createTestItem('9999999999999');
      
      const amount = await stockMovementService.getStockAmountByItem('9999999999999');
      expect(amount).toBe(0);
      
      const avgPrice = await stockMovementService.getAverageBuyPriceByItem('9999999999999');
      expect(avgPrice).toBe(0);
      
      const lastPrice = await stockMovementService.getLastBuyPriceByItem('9999999999999');
  ...
  describe('Update Operations', () => {
    ...
    it('should update stock movement via IPC wrapper', async () => {
      ...
      const updated = await stockMovementService.getOne('INV-500', '1234567890123');
  ...
  describe('Error Handling', () => {
    ...
    it('should handle stock movement not found on update', async () => {
      const response = await handleIpcRequest(() =>
        stockMovementService.update('INV-999', '1234567890123', { amount: '10' })
      );
