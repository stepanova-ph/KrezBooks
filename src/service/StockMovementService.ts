...
export class StockMovementService {
  ...
  async getOne(invoiceNumber: string, itemEan: string): Promise<StockMovement | undefined> {
    const db = getDatabase();
  ...
  async create(movement: CreateStockMovementInput): Promise<{ changes: number }> {
    const db = getDatabase();
  ...
  async update(
    ...
    const fieldsToUpdate = Object.keys(updates).filter(
      key => key !== 'invoice_number' && key !== 'item_ean' && key !== 'created_at'
    );
  ...
  async delete(invoiceNumber: string, itemEan: string): Promise<{ changes: number }> {
    const db = getDatabase();
