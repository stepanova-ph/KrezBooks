...
export function registerIpcHandlers() {
	...
	ipcMain.handle(
		...
		async (_event, items: { itemEan: string; newAmount: string }[]) => {
			return handleIpcRequest(() =>
				stockMovementService.shouldSetResetPointBatch(items),
			);
		},
	);
	...
	ipcMain.handle(
		...
		async (_event, itemEan: string) => {
			return handleIpcRequest(() =>
				stockMovementService.getByItemWithInvoiceInfo(itemEan),
			);
		},
	);
	...
	ipcMain.handle(
		"db:invoices:getOne",
		async (_event, prefix: string, number: string) => {
