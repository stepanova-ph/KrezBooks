...
export function registerIpcHandlers() {
  ...
  ipcMain.handle('db:contacts:update', async (_event, ico: string, modifier: number, updates: Partial<Contact>) => {
    return handleIpcRequest(() => contactService.update(ico, modifier, updates));
  });
  
  ipcMain.handle('db:contacts:delete', async (_event, ico: string, modifier: number) => {
