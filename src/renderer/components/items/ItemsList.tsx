...
import { formatPrice, formatVatRateShort } from "../../../utils/formattingUtils";
...
export const itemColumns: Column[] = [
  ...
  { id: 'sales_group', label: 'Skupina', minWidth: 15, align: 'center' as const },
  ...
  { id: 'sale_price_group1', label: 'Cena (Sk. 1)', minWidth: 80, align: 'right' as const },
  { id: 'sale_price_group2', label: 'Cena (Sk. 2)', minWidth: 80, align: 'right' as const },
  { id: 'sale_price_group3', label: 'Cena (Sk. 3)', minWidth: 80, align: 'right' as const },
  { id: 'sale_price_group4', label: 'Cena (Sk. 4)', minWidth: 80, align: 'right' as const },
  { id: 'note', label: 'Poznámka', hidden: true },
];

function ItemsList({ items, visibleColumnIds, columnOrder, onColumnOrderChange }: ItemsListProps) {
      ...
      <>
        <DataTable
          ...
          renderRow={(item, visibleColumns) => (
            <>
            {visibleColumns.filter(x => !x.hidden).map((column) => (
                <TableCell
                  key={column.id}
                  align={column.align}
