...
export const contactColumns: Column[] = [
  ...
  { id: 'modifier', label: 'Modifikátor', minWidth: 30, maxWidth: 30, align: 'center' as const, hide_label: true },
  ...
  { id: 'price_group', label: 'Cenová skupina', minWidth: 80, align: 'center' as const },
  { id: 'bank_account', label: 'Bankovní účet', minWidth: 100},
];

function ContactsList({ contacts, visibleColumnIds, columnOrder, onColumnOrderChange }: ContactsListProps) {
  const deleteContact = useDeleteContact();
  const [editingContact, setEditingContact] = useState<Contact | null>(null);

  const handleDelete = async (contact: Contact) => {
    try {
      await deleteContact.mutateAsync({ ico: contact.ico, modifier: contact.modifier });
    } catch (error) {
  ...
  const contextMenuActions: ContextMenuAction<Contact>[] = [
    ...
    },
    {
      ...
      requireConfirm: true,
      confirmMessage: (contact) => `Opravdu chcete smazat kontakt "${contact.company_name}"?`,
      divider: true,
    },
  ];
      ...
      case 'type':
        return (
          <Box display="flex" gap={0.5} flexWrap="wrap">
            {!!contact.is_customer && (
              <Chip label="Odběratel" size="small" color="primary" sx={{ height: 22 }} />
            )}
            {!!contact.is_supplier && (
              <Chip label="Dodavatel" size="small" color="secondary" sx={{ height: 22 }} />
            )}
        ...
        renderRow={(contact, visibleColumns) => (
          <>
            {visibleColumns.map((column) => (
              <TableCell key={column.id} align={column.align} style={{ maxWidth: column.maxWidth, minWidth: column.minWidth, width: column.width }}>
