...
function ContactForm({ open, onClose, onSubmit, initialData, mode, isPending = false }: ContactFormProps) {
  ...
  const [formData, setFormData] = useState<CreateContactInput>(
    initialData
      ? {
          ...defaultFormData,
          ...initialData,
          is_customer: typeof initialData.is_customer === 'boolean' ? initialData.is_customer : !!initialData.is_customer,
          is_supplier: typeof initialData.is_supplier === 'boolean' ? initialData.is_supplier : !!initialData.is_supplier,
        }
      : defaultFormData
  );
  ...
  const handleBlur = (fieldName: string) => {
    ...
    const dataToValidate = {
      ...formData,
      bank_account: combineBankAccount(bankAccountParts.accountNumber, bankAccountParts.bankCode),
  ...
  const handleBankAccountChange = (field: 'accountNumber' | 'bankCode') => (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setBankAccountParts(prev => {
      ...
      setFormData(prevForm => ({
        ...prevForm,
        bank_account: combineBankAccount(updated.accountNumber, updated.bankCode),
      }));
      return updated;
    });
  };

  const handleDICChange = (field: 'prefix' | 'value') => (e: React.ChangeEvent<HTMLInputElement | { value: unknown }>) => {
    const newValue = field === 'prefix' ? (e.target.value as string) : (e.target as HTMLInputElement).value;
    ...
    setDicParts(prev => {
      const updated = { ...prev, [field]: field === 'prefix' ? (newValue || null) : newValue };
    ...
    >
      <FormSection title="Základní údaje">
        ...
        <Box sx={{ display: 'flex', gap: 1, alignItems: 'flex-start' }}>
          {!isCustomDIC ? (
            <>
              <FormControl size="small" sx={{ minWidth: 80 }} error={!!errors.dic}>
              ...
              <TextFieldWithError
                ...
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleDICChange('value')(e)}
        ...
        {mode === 'edit' && (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: -1 }}>
            <Tooltip title={isUnlocked ? 'Zamknout IČO a DIČ' : 'Odemknout IČO a DIČ pro úpravu'}>
              <IconButton 
                size="small" 
        ...
        <Box sx={{ display: 'flex', gap: 3, alignItems: 'center' }}>
          ...
          {(errors.is_customer || errors.is_supplier) && (
            <Tooltip title={errors.is_customer || errors.is_supplier || ''} arrow>
              <ErrorOutlineIcon 
                sx={{ 
                  color: 'error.main',
                  fontSize: 20,
