...
import { formatNumber, parseNumericInput, clampNumber } from '../../../utils/formattingUtils';

interface NumberTextFieldProps extends Omit<TextFieldProps, 'type' | 'value' | 'onChange'> {
  name: string;
  value: number | string;
...
export function NumberTextField({
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    const isDecimal = (key === '.' || key === ',') && precision > 0 && !displayValue.includes('.');
