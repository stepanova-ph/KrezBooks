 ...
 */
export function sqliteIntegerToBoolean(value: number | null | undefined): boolean {
...
export function serializeContact(contact: Partial<Contact>): any {
  ...
  if ('is_supplier' in result) {
    result.is_supplier = booleanToSQLiteInteger(result.is_supplier as boolean) as any;
  }
  if ('is_customer' in result) {
    result.is_customer = booleanToSQLiteInteger(result.is_customer as boolean) as any;
  }
  return result;
}